...
export function HangoutPlanningModal({
  open,
  onOpenChange,
  contactId,
  contactName,
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-sm overflow-y-auto max-h-[90vh]">
        <DialogHeader>
          <DialogTitle>Plan a hangout with {contactName}</DialogTitle>
        </DialogHeader>
          ...
          {type === "in-person" && (
            <div>
              ...
              <LocationSearch
                ...
                onChange={(val) => {
                  setLocationName(val);
                  // Clear coords if user edits the field manually after a selection
