...
export function AddRecurringHangoutSheet({ open, onOpenChange, onSaved }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-sm max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {selected ? `Set up recurring hangout` : "Who do you want to see regularly?"}
          </DialogTitle>
        </DialogHeader>
        ...
        {!selected ? (
          ...
          <div className="pt-1">
            ...
            <ScheduleForm
              ...
              initialHangoutType={existingSchedule?.hangoutType ?? "in-person"}
              initialCadenceMode={existingSchedule?.cadenceMode ?? "perpetual"}
              initialLeadTimeDays={existingSchedule?.leadTimeDays ?? 7}
              initialDefaultHangout={
                existingSchedule?.defaultHangout
                  ? JSON.parse(existingSchedule.defaultHangout)
                  : null
              }
              initialNoteToFriend={existingSchedule?.noteToFriend ?? null}
              onSaved={handleSaved}
            />
          </div>
        )}
